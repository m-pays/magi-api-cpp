...
BOOST_AUTO_TEST_CASE(GetRawTransactionVerbose) {
	...
	BOOST_REQUIRE_NO_THROW(response = fx.xmg.getrawtransaction(txid,1));
...
BOOST_AUTO_TEST_CASE(DecodeRawTransaction) {
	...
	BOOST_REQUIRE_NO_THROW(response = fx.xmg.decoderawtransaction(rawHexTx));
...
BOOST_AUTO_TEST_CASE(SendRawTransaction) {
	...
	try{
		fx.xmg.sendrawtransaction(param);
...
BOOST_AUTO_TEST_CASE(CreateRawTransaction) {
	...
	BOOST_REQUIRE_NO_THROW(response = fx.xmg.createrawtransaction(param1, param2));
...
BOOST_AUTO_TEST_CASE(SignRawTransaction) {
	...
	BOOST_REQUIRE_NO_THROW(response = fx.xmg.signrawtransaction(param1,param2,param3,"ALL"));
...
BOOST_AUTO_TEST_CASE(GetRawMempool) {
	...
	BOOST_REQUIRE_NO_THROW(response = fx.xmg.getrawmempool());
...
BOOST_AUTO_TEST_CASE(GetRawChangeAddress) {
	...
	std::string response;
	BOOST_REQUIRE_NO_THROW(response = fx.xmg.getrawchangeaddress());
