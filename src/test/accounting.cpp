...
BOOST_AUTO_TEST_CASE(GetBalance) {
	...
	BOOST_REQUIRE_NO_THROW(fx.xmg.getbalance());
	BOOST_REQUIRE_NO_THROW(fx.xmg.getbalance(""));

	#ifdef VERBOSE
	double response = fx.xmg.getbalance();
...
BOOST_AUTO_TEST_CASE(GetReceivedByAccount) {
	...
	BOOST_REQUIRE_NO_THROW(fx.xmg.getreceivedbyaccount(""));

	#ifdef VERBOSE
	double response = fx.xmg.getreceivedbyaccount("");
...
BOOST_AUTO_TEST_CASE(GetReceivedByAddress) {
	...
	BOOST_REQUIRE_NO_THROW(fx.xmg.getreceivedbyaddress(param));

	#ifdef VERBOSE
	double response = fx.xmg.getreceivedbyaddress(param);
...
BOOST_AUTO_TEST_CASE(ListReceivedByAccount) {
	...
	BOOST_REQUIRE_NO_THROW(response = fx.xmg.listreceivedbyaccount(1,true));
...
BOOST_AUTO_TEST_CASE(ListReceivedByAddress) {
	...
	BOOST_REQUIRE_NO_THROW(response = fx.xmg.listreceivedbyaddress(1,true));
...
BOOST_AUTO_TEST_CASE(GetTransaction) {
	...
	BOOST_REQUIRE_NO_THROW(response = fx.xmg.gettransaction(param));
...
BOOST_AUTO_TEST_CASE(ListTransactions) {
	...
	BOOST_REQUIRE_NO_THROW(response = fx.xmg.listtransactions());
...
BOOST_AUTO_TEST_CASE(GetAccount) {
	...
	std::string address;
	std::string response;
	BOOST_REQUIRE_NO_THROW(address = fx.xmg.getnewaddress("TestUser"));
	BOOST_REQUIRE_NO_THROW(response = fx.xmg.getaccount(address));
...
BOOST_AUTO_TEST_CASE(GetAddressesByAccount) {
	...
	BOOST_REQUIRE_NO_THROW(response = fx.xmg.getaddressesbyaccount("TestUser"));
...
BOOST_AUTO_TEST_CASE(ListAccounts) {
	...
	BOOST_REQUIRE_NO_THROW(response = fx.xmg.listaccounts());
...
/*
	...
	BOOST_REQUIRE_NO_THROW(acc1_old = fx.xmg.getbalance("Bitonic"));
	BOOST_REQUIRE_NO_THROW(acc2_old = fx.xmg.getbalance("Bitonic 2"));

	BOOST_REQUIRE_NO_THROW(response = fx.xmg.move("Bitonic", "Bitonic 2", 0.001));
	BOOST_REQUIRE(response == true);

	BOOST_REQUIRE_NO_THROW(acc1_new = fx.xmg.getbalance("Bitonic"));
	BOOST_REQUIRE_NO_THROW(acc2_new = fx.xmg.getbalance("Bitonic 2"));
...
BOOST_AUTO_TEST_CASE(SetAccount) {
	...
	std::string address;
	BOOST_REQUIRE_NO_THROW(address = fx.xmg.getnewaddress("TestUser"))
	BOOST_REQUIRE_NO_THROW(fx.xmg.setaccount(address,"TestUser2"));
}
...
/*
	...
	BOOST_REQUIRE_NO_THROW(addr = fx.xmg.getnewaddress("TestUser"));
	BOOST_REQUIRE_NO_THROW(fx.xmg.walletpassphrase("123456", 5));
	BOOST_REQUIRE_NO_THROW(txid = fx.xmg.sendtoaddress(addr, 0.0001));
...
/*
	...
	BOOST_REQUIRE_NO_THROW(fx.xmg.walletpassphrase("123456", 3));
	BOOST_REQUIRE_NO_THROW(response = fx.xmg.sendfrom("Bitonic 2", "1DvwT9U88mLKUztzrXqwZugqYFasr97oLe", 0.0002));
...
/*
	...
	BOOST_REQUIRE_NO_THROW(fx.xmg.walletpassphrase("123456", 3));
	BOOST_REQUIRE_NO_THROW(response = fx.xmg.sendmany("Bitonic 2", param));
	BOOST_REQUIRE(response.size() == 64);
...
BOOST_AUTO_TEST_CASE(GetTxOut) {
	...
	BOOST_REQUIRE_NO_THROW(response = fx.xmg.gettxout(txid, 0));
...
BOOST_AUTO_TEST_CASE(GetTxOutSetInfo) {
	...
	BOOST_REQUIRE_NO_THROW(response = fx.xmg.gettxoutsetinfo());
...
BOOST_AUTO_TEST_CASE(ListUnspent) {
	...
	BOOST_REQUIRE_NO_THROW(response = fx.xmg.listunspent());
...
BOOST_AUTO_TEST_CASE(ListLockUnspent) {
	...
	BOOST_REQUIRE_NO_THROW(response = fx.xmg.listlockunspent());
...
BOOST_AUTO_TEST_CASE(LockUnspent) {
	...
	BOOST_REQUIRE_NO_THROW(response = fx.xmg.lockunspent(false, param));
	BOOST_REQUIRE(response == true);
}
...
BOOST_AUTO_TEST_CASE(ListAddressGroupings) {
	...
	BOOST_REQUIRE_NO_THROW(response = fx.xmg.listaddressgroupings());
